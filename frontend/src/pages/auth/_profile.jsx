    ...
    for (let i = 0; i <= latestBlock; i++) {
      ...
      if (block && block.transactions) {
        let filteredTx = block.transactions
          ...
          .filter(tx => tx.value && web3Instance.utils.fromWei(tx.value, "ether") > 0)
          .map(tx => ({
            ...tx,
    ...
    <div className="min-h-screen bg-gray-50 pt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500">Quản lý thông tin và lịch sử tiêm chủng của bạn</p>
        </div>
            ...
            <Card>
              <Tabs defaultActiveKey="1">
                {user?.roleName === 'PATIENT' && (
                  <TabPane
                    tab={<span><CalendarOutlined />Lịch sử đăng ký tiêm chủng</span>}
                    key="2"
                  >
                    <AppointmentHistory
                ...
                <TabPane
                  tab={<span><WalletOutlined />Lịch sử giao dịch</span>}
                  key="3"
                >
                  <TransactionHistory
